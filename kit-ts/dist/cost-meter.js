...
const pricing_json_1 = __importDefault(require("../data/pricing.json"));
function loadPricing(path) {
    if (path === undefined) {
        return pricing_json_1.default;
    }
...
function getModelPricing(model, pricing) {
...
class CostMeter {
    ...
    countMessages(messages) {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            const tc = { turn_index: i, role: msg.role, tokens, cost_usd };
            perTurn.push(tc);
            total += tokens;
            this.onTurn?.(tc);
        }
