...
function HashDisplay({ hash }: { hash: string }) {
  ...
  return (
    <div className="flex items-center gap-2 font-mono text-xs text-brand-accent bg-brand-statist border border-brand-border rounded-lg p-3 overflow-x-auto">
        ...
        <Section title="Conditions Evaluated">
          <div className="divide-y divide-brand-border rounded-lg border border-brand-border overflow-hidden">
            {conditions.map(([key, cond]) => (
              <div key={key} className="flex items-center gap-4 px-4 py-3 bg-brand-surface">
        ...
        <Section title="Entity State at Evaluation">
          <div className="divide-y divide-brand-border rounded-lg border border-brand-border overflow-hidden">
            {Object.entries(receipt.entity_state_snapshot).map(([k, v]) => (
              <div key={k} className="flex justify-between items-center px-4 py-2.5 bg-brand-surface">
        ...
        {receipt.executed_at ? (
          <div className="space-y-3">
            ...
            {receipt.execution_result && (
              <pre className="rounded-lg border border-brand-border bg-brand-statist p-3 text-xs font-mono text-brand-accent overflow-auto max-h-40">
