...
import { signUpWithOAuth, signUpWithEmail } from "./actions";

export const metadata = { title: "Create an account · Statis" };

type InviteCtx = { token: string; email?: string; orgName?: string };

async function readInvite(): Promise<InviteCtx | null> {
  const store = await cookies();
  const raw = store.get("statis_invite")?.value;
  if (!raw) return null;
  try {
...
export default async function SignupPage() {
  ...
  const invitedOrg = invite?.orgName ?? "your team";
  ...
  return (
    <div className="w-full max-w-[360px] bg-paper border border-rule rounded-[4px] p-7 shadow-[0_1px_0_rgba(0,0,0,0.02),0_6px_16px_-8px_rgba(60,40,20,0.1)]">
      ...
      {invite ? (
        <div className="bg-[rgba(29,58,46,0.06)] border border-seal border-l-2 border-l-seal rounded-[3px] px-3 py-2.5 mb-4 text-[12px] leading-[1.5] text-ink tracking-[-0.005em]">
          <span className="block font-mono text-[9.5px] tracking-[0.14em] uppercase text-seal font-medium mb-1">
            ◈ Accepting invite
          </span>
          You&apos;ll join <strong className="font-medium">{invitedOrg}</strong> as soon as your account is
          created. No org setup needed.
        </div>
      ) : null}

      <h1 className="font-sans font-medium text-[20px] tracking-[-0.025em] leading-tight text-center text-ink mb-1.5">
        {invite ? `Join ${invitedOrg}.` : "Create an account."}
      </h1>
      <p className="text-[13px] leading-[1.55] text-ink-soft tracking-[-0.005em] text-center mb-[22px]">
        {invite ? "One step to finish accepting your invite." : "Governance infrastructure for agents."}
      </p>

      <form action={async () => { "use server"; await signUpWithOAuth("GoogleOAuth"); }}>
      ...
      <form action={async () => { "use server"; await signUpWithOAuth("GitHubOAuth"); }}>
        <button type="submit" className="w-full flex items-center justify-center gap-2.5 py-2 px-3 bg-paper border border-rule rounded-[3px] text-[13px] text-ink tracking-[-0.005em] hover:bg-bg hover:border-ink-muted transition-colors mb-2">
      ...
      <form action={signUpWithEmail}>
        <div className="mb-3">
          <label htmlFor="email" className="block font-mono text-[9.5px] tracking-[0.14em] uppercase text-ink-muted mb-[5px]">
            {invite ? "Invited email" : "Work email"}
          </label>
          <input
            id="email"
            name="email"
            ...
            className={
              "w-full py-2 px-[11px] bg-paper border border-rule rounded-[3px] font-sans text-[13px] text-ink tracking-[-0.005em] placeholder:text-ink-muted focus:outline-none focus:border-accent focus:ring-2 focus:ring-[rgba(184,68,46,0.15)] " +
              (invitedEmail ? "bg-bg text-ink-soft cursor-not-allowed" : "")
            }
          />
          {invitedEmail ? (
            <p className="text-[11px] leading-[1.4] text-ink-muted mt-1 tracking-[-0.005em]">
              Locked to the email your admin invited. Use that mailbox to finish signup.
            </p>
          ) : null}
        </div>
        <button
      ...
      <div className="text-center text-[12px] text-ink-muted mt-[18px] tracking-[-0.005em] leading-[1.55]">
        ...
        <span className="text-[10.5px] text-ink-muted">
          ...
          <a href="https://statis.dev/terms" className="text-ink-soft border-b border-dotted border-ink-muted hover:text-accent hover:border-accent">Terms</a>
          {" "}and{" "}
          <a href="https://statis.dev/privacy" className="text-ink-soft border-b border-dotted border-ink-muted hover:text-accent hover:border-accent">Privacy Policy</a>
          .
        </span>
      </div>
    </div>
